...
- ✓ Orchestra dispatches GSD commands via `claude -p` into instrument directories (synchronous, stdout captured) — v3.0
- ✓ Orchestra runs parallel `claude -p` across multiple instruments simultaneously — v3.0
- ✓ Remove claude-peers MCP server, broker, message-watcher from instruments and orchestra — v3.0
- ✓ Remove CLAUDE_CHANNELS env var and `--dangerously-load-development-channels` flag from wrapper — v3.0
- ✓ Fix duplicate "General coding session" appearing on phone from pre-created sessions — v3.0
- ✓ Deploy GSD and superpowers skills to VPS so instruments have `/gsd:*` commands — v3.0
- ✓ Instruments know their own instance name via CLAUDE.md or env injection — v3.0
- ✓ Orchestra CLAUDE.md rewritten for `claude -p` dispatch architecture — v3.0
- ✓ `claude -p --continue` for multi-step GSD sequences, fresh context by default — v3.0
- ✓ Orchestra handles long-running `claude -p` tasks (execute-phase can take minutes) — v3.0
...
| FIFO stdin for remote-control (same as telegram) | Unified pattern, heartbeat prevents session death | ✓ Good |
| Clear ANTHROPIC_API_KEY for remote-control mode | API key overrides OAuth auth; remote-control requires Max subscription OAuth | ✓ Good |
| Replace claude-peers with `claude -p` dispatch | Peer messaging required broker, watcher, peek/ack — all workarounds for channel notifications being silently dropped in remote-control. `claude -p` is synchronous, proven, zero infrastructure | ✓ Good |
| Fresh context by default, `--continue` for continuity | Each `claude -p` is a clean slate. GSD state lives in `.planning/` files. Use `--continue` only for multi-step sequences | ✓ Good |
...
| deploy_skills in Linux path only | macOS is dev-only, instruments are Linux-only. Skills already present on dev machine | ✓ Good |
| Identity CLAUDE.md in .claude/ not repo root | Avoids conflicting with project CLAUDE.md; .claude/ is user-level config | ✓ Good |
